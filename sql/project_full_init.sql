...
-- =========================================================
-- Stock and fund tables
-- =========================================================

CREATE TABLE IF NOT EXISTS `stocks` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `stock_code` VARCHAR(16) NOT NULL COMMENT 'Stock code',
  `stock_name` VARCHAR(64) NOT NULL COMMENT 'Stock name',
  `market` VARCHAR(10) NOT NULL COMMENT 'Market',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Stocks';

CREATE TABLE IF NOT EXISTS `stock_quotes` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `stock_id` BIGINT NOT NULL COMMENT 'Stock id',
  `current_price` DECIMAL(12, 4) NOT NULL COMMENT 'Current price',
  ...
  `turnover` DECIMAL(18, 2) DEFAULT NULL COMMENT 'Turnover',
  `change_amount` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Price change',
  `change_percent` DECIMAL(8, 4) DEFAULT NULL COMMENT 'Change percent',
  `quote_time` DATETIME NOT NULL COMMENT 'Quote time',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_stock_quotes_stock_id` (`stock_id`),
  KEY `idx_stock_quotes_quote_time` (`quote_time`),
  KEY `idx_stock_quotes_stock_time` (`stock_id`, `quote_time`),
  CONSTRAINT `fk_stock_quotes_stock` FOREIGN KEY (`stock_id`) REFERENCES `stocks` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Stock quotes';

CREATE TABLE IF NOT EXISTS `stock_analysis` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `stock_id` BIGINT NOT NULL COMMENT 'Stock id',
  `analysis_time` DATETIME NOT NULL COMMENT 'Analysis time',
  `analysis_content` TEXT NOT NULL COMMENT 'Analysis content',
  `trend_type` VARCHAR(20) DEFAULT NULL COMMENT 'UP/DOWN/STABLE',
  `confidence_score` DECIMAL(6, 2) DEFAULT NULL COMMENT 'Confidence',
  `predicted_price` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Predicted price',
  `risk_level` VARCHAR(20) DEFAULT NULL COMMENT 'LOW/MEDIUM/HIGH',
  `ai_provider` VARCHAR(20) DEFAULT NULL COMMENT 'AI provider',
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '1 valid, 0 invalid',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_stock_analysis_stock_id` (`stock_id`),
  KEY `idx_stock_analysis_time` (`analysis_time`),
  KEY `idx_stock_analysis_provider` (`ai_provider`),
  CONSTRAINT `fk_stock_analysis_stock` FOREIGN KEY (`stock_id`) REFERENCES `stocks` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Stock AI analysis';

CREATE TABLE IF NOT EXISTS `user_stock_watch` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `user_id` BIGINT NOT NULL COMMENT 'User id',
  `stock_id` BIGINT NOT NULL COMMENT 'Stock id',
  `watch_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Watch time',
  `alert_enabled` TINYINT NOT NULL DEFAULT 0 COMMENT '1 enabled, 0 disabled',
  `alert_price` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Alert price',
  `alert_type` VARCHAR(20) DEFAULT NULL COMMENT 'UP/DOWN',
  `notes` VARCHAR(500) DEFAULT NULL COMMENT 'Notes',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_stock_watch` (`user_id`, `stock_id`),
  KEY `idx_user_stock_watch_stock_id` (`stock_id`),
  CONSTRAINT `fk_user_stock_watch_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_user_stock_watch_stock` FOREIGN KEY (`stock_id`) REFERENCES `stocks` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='User stock watchlist';

CREATE TABLE IF NOT EXISTS `fund_history` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `fund_code` VARCHAR(20) NOT NULL COMMENT 'Fund code',
  `fund_name` VARCHAR(100) NOT NULL COMMENT 'Fund name',
  `net_value` DECIMAL(12, 4) NOT NULL COMMENT 'Net value',
  `accumulated_value` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Accumulated value',
  `change_percent` DECIMAL(10, 4) DEFAULT 0.0000 COMMENT 'Change percent',
  `trade_date` DATE NOT NULL COMMENT 'Trade date',
  `create_time` DATE NOT NULL COMMENT 'Create date',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_fund_history_code_date` (`fund_code`, `trade_date`),
  KEY `idx_fund_history_trade_date` (`trade_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Fund history';

CREATE TABLE IF NOT EXISTS `fund_ranking` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `fund_code` VARCHAR(20) NOT NULL COMMENT 'Fund code',
  `fund_name` VARCHAR(100) NOT NULL COMMENT 'Fund name',
  `net_value` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Net value',
  `accumulated_value` DECIMAL(12, 4) DEFAULT NULL COMMENT 'Accumulated value',
  `change_percent` DECIMAL(10, 4) DEFAULT NULL COMMENT 'Daily change percent',
  `one_week_return` DECIMAL(10, 4) DEFAULT NULL COMMENT '1 week return',
  `one_month_return` DECIMAL(10, 4) DEFAULT NULL COMMENT '1 month return',
  `three_month_return` DECIMAL(10, 4) DEFAULT NULL COMMENT '3 month return',
  `six_month_return` DECIMAL(10, 4) DEFAULT NULL COMMENT '6 month return',
  `one_year_return` DECIMAL(10, 4) DEFAULT NULL COMMENT '1 year return',
  `ytd_return` DECIMAL(10, 4) DEFAULT NULL COMMENT 'Year to date return',
  `since_inception_return` DECIMAL(10, 4) DEFAULT NULL COMMENT 'Since inception return',
  `max_drawdown` DECIMAL(10, 4) DEFAULT NULL COMMENT 'Max drawdown',
  `sharpe_ratio` DECIMAL(10, 4) DEFAULT NULL COMMENT 'Sharpe ratio',
  `manager_name` VARCHAR(100) DEFAULT NULL COMMENT 'Manager name',
  `manager_years` INT DEFAULT NULL COMMENT 'Manager years',
  `star_rating` INT DEFAULT NULL COMMENT 'Star rating',
  `fund_type` VARCHAR(50) DEFAULT NULL COMMENT 'Fund type',
  `sector` VARCHAR(100) DEFAULT NULL COMMENT 'Sector',
  `update_date` DATE NOT NULL COMMENT 'Update date',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_fund_ranking_code_date` (`fund_code`, `update_date`),
  KEY `idx_fund_ranking_update_date` (`update_date`),
  KEY `idx_fund_ranking_one_year_return` (`one_year_return`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Fund ranking snapshots';

CREATE TABLE IF NOT EXISTS `fund_news` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  `title` VARCHAR(300) NOT NULL COMMENT 'News title',
  `summary` TEXT DEFAULT NULL COMMENT 'News summary',
  `url` VARCHAR(500) DEFAULT NULL COMMENT 'News URL',
  `publish_date` DATETIME DEFAULT NULL COMMENT 'Publish datetime',
  `update_date` DATE DEFAULT NULL COMMENT 'Snapshot date',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_fund_news_title_publish` (`title`, `publish_date`),
  KEY `idx_fund_news_update_date` (`update_date`),
  KEY `idx_fund_news_publish_date` (`publish_date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Fund news';
...
VALUES
  ('sh600000', 'PF Bank', 'SH', 'Banking', 1),
  ('sh600519', 'Moutai', 'SH', 'Liquor', 1),
  ('sz000001', 'Ping An Bank', 'SZ', 'Banking', 1),
  ('sz300750', 'CATL', 'SZ', 'New Energy', 1)
ON DUPLICATE KEY UPDATE
...
INSERT INTO `stock_quotes`
  (`、、、、`, `current_price`, `open_price`, `close_price`, `high_price`, `low_price`, `volume`, `turnover`, `change_amount`, `change_percent`, `quote_time`)
SELECT s.id, 10.5600, 10.3000, 10.1200, 10.6500, 10.1000, 123456789, 1300000000.00, 0.4400, 4.3478, NOW()
FROM `stocks` s
WHERE s.stock_code = 'sh600000'
  AND NOT EXISTS (
    SELECT 1 FROM `stock_quotes` q WHERE q.stock_id = s.id
  );

INSERT INTO `stock_analysis`
  (`stock_id`, `analysis_time`, `analysis_content`, `trend_type`, `confidence_score`, `predicted_price`, `risk_level`, `ai_provider`, `status`)
SELECT s.id, NOW(), 'Sample upward trend analysis generated for bootstrap.', 'UP', 78.50, 10.9800, 'MEDIUM', 'qwen', 1
FROM `stocks` s
WHERE s.stock_code = 'sh600000'
  AND NOT EXISTS (
    SELECT 1 FROM `stock_analysis` a WHERE a.stock_id = s.id
  );

INSERT INTO `user_stock_watch`
  (`user_id`, `stock_id`, `watch_time`, `alert_enabled`, `alert_price`, `alert_type`, `notes`)
SELECT u.id, s.id, NOW(), 1, 11.0000, 'UP', 'Bootstrap watchlist item'
FROM `users` u
JOIN `stocks` s ON s.stock_code = 'sh600000'
WHERE u.username = 'admin'
  AND NOT EXISTS (
    SELECT 1 FROM `user_stock_watch` w WHERE w.user_id = u.id AND w.stock_id = s.id
  );

INSERT INTO `fund_ranking`
  (`fund_code`, `fund_name`, `net_value`, `accumulated_value`, `change_percent`, `one_week_return`, `one_month_return`,
   `three_month_return`, `six_month_return`, `one_year_return`, `ytd_return`, `since_inception_return`,
   `max_drawdown`, `sharpe_ratio`, `manager_name`, `manager_years`, `star_rating`, `fund_type`, `sector`, `update_date`)
VALUES
  ('000001', 'Sample Growth Fund', 1.3560, 2.8450, 0.5200, 1.1200, 3.5600, 8.2300, 12.8800, 18.3500, 5.6800, 128.5000, 10.2500, 1.4200, 'Alice Lee', 5, 4, 'Hybrid', 'Technology', CURDATE()),
  ('000002', 'Sample Index Fund', 1.1280, 2.1100, -0.1200, 0.3400, 1.8200, 5.6200, 9.4300, 13.2600, 4.1200, 96.3000, 8.7400, 1.1800, 'Bob Wang', 7, 5, 'Index', 'Broad Market', CURDATE())
ON DUPLICATE KEY UPDATE
  `fund_name` = VALUES(`fund_name`),
  `net_value` = VALUES(`net_value`),
  `accumulated_value` = VALUES(`accumulated_value`),
  `change_percent` = VALUES(`change_percent`),
  `one_week_return` = VALUES(`one_week_return`),
  `one_month_return` = VALUES(`one_month_return`),
  `three_month_return` = VALUES(`three_month_return`),
  `six_month_return` = VALUES(`six_month_return`),
  ...
  `manager_years` = VALUES(`manager_years`),
...
INSERT INTO `fund_history`
  (`fund_code`, `fund_name`, `net_value`, `accumulated_value`, `change_percent`, `trade_date`, `create_time`)
VALUES
  ('000001', 'Sample Growth Fund', 1.3200, 2.8000, 0.3500, CURDATE() - INTERVAL 2 DAY, CURDATE()),
  ('000001', 'Sample Growth Fund', 1.3410, 2.8260, 1.5900, CURDATE() - INTERVAL 1 DAY, CURDATE()),
  ('000001', 'Sample Growth Fund', 1.3560, 2.8450, 1.1200, CURDATE(), CURDATE()),
  ('000002', 'Sample Index Fund', 1.1210, 2.1000, -0.1500, CURDATE() - INTERVAL 2 DAY, CURDATE()),
  ('000002', 'Sample Index Fund', 1.1290, 2.1110, 0.7100, CURDATE() - INTERVAL 1 DAY, CURDATE()),
  ('000002', 'Sample Index Fund', 1.1280, 2.1100, -0.0900, CURDATE(), CURDATE())
ON DUPLICATE KEY UPDATE
  `fund_name` = VALUES(`fund_name`),
  `net_value` = VALUES(`net_value`),
  `accumulated_value` = VALUES(`accumulated_value`),
...
INSERT INTO `fund_news` (`title`, `summary`, `url`, `publish_date`, `update_date`)
VALUES
  ('Market sentiment improves for equity funds', 'Sample bootstrap news item for the fund module.', 'https://example.com/fund-news-1', CONCAT(CURDATE() - INTERVAL 1 DAY, ' 09:00:00'), CURDATE() - INTERVAL 1 DAY),
  ('Index funds see steady inflows', 'Another sample bootstrap news item for the fund module.', 'https://example.com/fund-news-2', CONCAT(CURDATE() - INTERVAL 1 DAY, ' 15:30:00'), CURDATE() - INTERVAL 1 DAY)
ON DUPLICATE KEY UPDATE
