...
public class AiChatServiceImpl implements AiChatService {
    ...
    private AiChatResponse chatWithQwen(AiChatRequest request, long startTime) {
        ...
        try {
            Map<String, Object> requestBody = new HashMap<>();
            requestBody.put("model", request.getModel() != null ? request.getModel() : aiProperties.getQwen().getModel());
            ...
            parameters.put("max_tokens", request.getMaxTokens() != null ? request.getMaxTokens() : aiProperties.getQwen().getMaxTokens());
            parameters.put("temperature", request.getTemperature() != null ? request.getTemperature() : aiProperties.getQwen().getTemperature());
            requestBody.put("parameters", parameters);
            
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.setBearerAuth(aiProperties.getQwen().getApiKey());
            
            HttpEntity<Map<String, Object>> entity = new HttpEntity<>(requestBody, headers);
            
            ResponseEntity<String> response = restTemplate.exchange(
                aiProperties.getQwen().getApiUrl(),
                HttpMethod.POST,
                entity,
                String.class
            );
