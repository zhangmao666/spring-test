package com.example.springboottest.modules.file.service.impl;
    ...
    public boolean deleteFile(String fileName) {
        try {
            minioClient.removeObject(
                    RemoveObjectArgs.builder().bucket(minioProperties.getBucketName()).object(fileName).build()
            );
    ...
    public FileListResponse listFiles(String prefix) {
        try {
            ...
            Iterable<Result<Item>> results = minioClient.listObjects(
                    ListObjectsArgs.builder().bucket(minioProperties.getBucketName()).prefix(prefix).build()
            );
            for (Result<Item> result : results) {
                Item item = result.get();
                String fileUrl = getFileUrl(item.objectName(), 3600);
                files.add(new FileListResponse.FileInfo(item.objectName(), item.size(),
                        item.lastModified().toString(), fileUrl, "application/octet-stream"));
            }
