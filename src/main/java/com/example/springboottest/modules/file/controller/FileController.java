...
public class FileController {
    ...
    public ApiResponse<FileUploadResponse> uploadFile(
        ...
        try {
            ...
            if (file.getSize() > 100 * 1024 * 1024) return ApiResponse.error("文件大小不能超过100MB");
    ...
    public ResponseEntity<?> downloadFile(@PathVariable String fileName) {
        try {
            if (!fileService.fileExists(fileName)) return ResponseEntity.notFound().build();
    ...
    public ApiResponse<Boolean> deleteFile(@PathVariable String fileName) {
        try {
            if (!fileService.fileExists(fileName)) return ApiResponse.error("文件不存在");
    ...
    @GetMapping("/list")
    public ApiResponse<FileListResponse> listFiles(@RequestParam(value = "prefix", required = false) String prefix) {
        try {
    ...
    public ApiResponse<String> getFileUrl(@PathVariable String fileName, @RequestParam(value = "expiry", defaultValue = "3600") Integer expiry) {
        try {
            if (!fileService.fileExists(fileName)) return ApiResponse.error("文件不存在");
