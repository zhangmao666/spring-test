...
-- 使用数据�?USE spring_boot_test;

-- 创建用户�?CREATE TABLE IF NOT EXISTS users (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '用户ID',
    username VARCHAR(50) NOT NULL UNIQUE COMMENT '用户�?,
    password VARCHAR(255) NOT NULL COMMENT '密码',
    email VARCHAR(100) NOT NULL UNIQUE COMMENT '邮箱',
    status INT NOT NULL DEFAULT 1 COMMENT '状态：1启用�?禁用',
...
-- 插入测试用户（密码是 123456 的BCrypt加密结果�?INSERT INTO users (username, password, email, status) VALUES 
...
-- 创建设备类型�?CREATE TABLE IF NOT EXISTS device_type (
    device_type_code VARCHAR(40) PRIMARY KEY COMMENT '设备类型代码',
    name VARCHAR(40) COMMENT '名称',
    status TINYINT COMMENT '状态：1启用�?禁用',
...
('SENSOR_001', '温度传感�?, 1, '高精度温度传感器', 'TEMP', '/icons/temperature.png', '用于监测环境温度的高精度传感�?, NOW(), 1, NOW(), 1, '/icons/pc/temperature.png', '#FF5722', '/images/temp_sensor.jpg'),
('SENSOR_002', '湿度传感�?, 1, '数字湿度传感�?, 'HUMI', '/icons/humidity.png', '用于监测空气湿度的数字传感器', NOW(), 1, NOW(), 1, '/icons/pc/humidity.png', '#2196F3', '/images/humi_sensor.jpg'),
('CAMERA_001', '监控摄像�?, 1, '高清网络摄像�?, 'CAM', '/icons/camera.png', '1080P高清网络监控摄像�?, NOW(), 1, NOW(), 1, '/icons/pc/camera.png', '#4CAF50', '/images/camera.jpg')
ON DUPLICATE KEY UPDATE 
...
-- 插入测试API客户�?-- 注意：这里的client_secret�?"123456" 的BCrypt加密结果
-- 测试客户端凭证：
...
('client_test_demo_001', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iKTVajIK', '测试客户�?, 'read,write', 7200000, 1, '用于开发测试的API客户�?)
ON DUPLICATE KEY UPDATE 
client_id = VALUES(client_id);

-- 创建字典�?CREATE TABLE IF NOT EXISTS dict (
    ...
    status TINYINT NOT NULL DEFAULT 1 COMMENT '状态：1启用�?禁用',
...
CREATE TABLE IF NOT EXISTS dict_item (
    ...
    item_label VARCHAR(100) NOT NULL COMMENT '字典项标�?,
    item_value VARCHAR(100) NOT NULL COMMENT '字典项�?,
...
('user_status', '用户状�?, '用户账户状态字�?, 1, 1, 1),
('device_status', '设备状�?, '设备运行状态字�?, 1, 1, 1),
('gender', '性别', '性别字典', 1, 1, 1)
ON DUPLICATE KEY UPDATE
dict_code = VALUES(dict_code);

-- 插入测试字典项数�?INSERT INTO dict_item (dict_id, item_label, item_value, item_sort, status, description, create_by, update_by) VALUES
-- 用户状�?((SELECT id FROM dict WHERE dict_code = 'user_status'), '启用', '1', 1, 1, '用户账户正常启用', 1, 1),
((SELECT id FROM dict WHERE dict_code = 'user_status'), '禁用', '0', 2, 1, '用户账户已禁�?, 1, 1),
((SELECT id FROM dict WHERE dict_code = 'user_status'), '锁定', '-1', 3, 1, '用户账户已锁�?, 1, 1),
-- 设备状�?((SELECT id FROM dict WHERE dict_code = 'device_status'), '在线', 'online', 1, 1, '设备在线运行', 1, 1),
...
((SELECT id FROM dict WHERE dict_code = 'device_status'), '维护', 'maintenance', 4, 1, '设备维护�?, 1, 1),
-- 性别
((SELECT id FROM dict WHERE dict_code = 'gender'), '�?, 'male', 1, 1, '男�?, 1, 1),
((SELECT id FROM dict WHERE dict_code = 'gender'), '�?, 'female', 2, 1, '女�?, 1, 1),
